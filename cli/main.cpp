 ...
 * @LastEditors: Leo
 * @LastEditTime: 2022-02-09 17:18:26
 */
...
void append_op(int argc, char *argv[]) {
  ...
  append_parser.add<string>("type", 't', "protocol type", true, "str",
...
void create_op(int argc, char *argv[]) {
  ...
  if (!create_parser.parse(argc, argv)) {
    ...
    std::cout << "--Create" << std::endl;
    auto channel = grpc::CreateChannel("localhost:9901",
...
int main(int argc, char *argv[]) {
  ...
  std::vector<
      std::pair<std::string, std::function<void(int argc, char *argv[])>>>
      ops{std::pair("Create", std::bind(create_op, std::placeholders::_1,
                                        std::placeholders::_2)),
          std::pair("Append", std::bind(append_op, std::placeholders::_1,
                                        std::placeholders::_2))};
