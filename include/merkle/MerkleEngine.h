 ...
 * @LastEditTime: 2022-07-22 09:19:08
 * @LastEditors: Leo
...
public:
   ...
   * leaf meta and error
   */
  std::pair<merkle_engine::LeafMeta, std::optional<common::Error>>
  Add(const std::string &hash)
  {
    ...
    if (is_full())
    {
      ...
      if (rocksdb::Status status =
              db_->Put(rocksdb::WriteOptions(), kMerkleRoot,
                       std::string(root_hash.begin(), root_hash.end()));
          !status.ok())
      {
