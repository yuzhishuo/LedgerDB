 ...
 * @LastEditTime: 2022-07-22 09:50:30
 * @LastEditors: Leo
 * @Description:
...
void __peer_msg_send(peer_connection_t *conn, tpl_node *tn, Buffer *buf, char *data);
...
int __offer_cfg_change(raft_server_t *raft, const unsigned char *data, raft_logtype_e change_type)
{
  ...
  {
    raft_remove_node(raft, raft_get_node((raft_server_t *)sv->raft, change->node_id));
    if (conn)
      conn->node = NULL;
...
static size_t __peer_msg_serialize(tpl_node *tn, muduo::net::Buffer *buf, char *data)
{
  size_t sz;
  tpl_pack(tn, 0);
...
bool operator==(const muduo::net::InetAddress &left, const muduo::net::InetAddress &right)
{
...
int __send_handshake_response(int node_id, peer_connection_t *conn, handshake_state_e success, raft_node_t *leader,
                              int16_t http_port)
{
  ...
  if (leader)
  {
    if (auto leader_conn = (peer_connection_t *)raft_node_get_udata(leader); leader_conn)
    {
      msg.hsr.leader_port = leader_conn->raft_port;
      snprintf(msg.hsr.leader_host, IP_STR_LEN, "%s", leader_conn->addr.toIpPort().data());
    }
  }
...
int __deserialize_and_handle_msg(void *img, size_t sz, void *data)
{
  ...
  if (0 < conn->n_expected_entries)
  {
    ...
    e = raft_recv_appendentries((raft_server_t *)(self->raft), conn->node, &conn->ae.ae, &msg.aer);
  ...
  case MSG_HANDSHAKE:
  {
    ...
    raft_node_t *leader = raft_get_current_leader_node((raft_server_t *)self->raft);

    /* Is this peer in our configuration already? */
    raft_node_t *node = raft_get_node((raft_server_t *)self->raft, m.hs.node_id);
    if (node)
    {
    ...
    if (!leader)
    {
      return __send_handshake_response(self->node_id, conn, HANDSHAKE_FAILURE, NULL, self->http_port);
    }
    else if (raft_node_get_id(leader) != self->node_id)
    {
      return __send_handshake_response(self->node_id, conn, HANDSHAKE_FAILURE, leader, self->http_port);
    }
    else if (node)
    {
      return __send_handshake_response(self->node_id, conn, HANDSHAKE_SUCCESS, NULL, self->http_port);
    }
    else
    {
      int e = __append_cfg_change(self, RAFT_LOGTYPE_ADD_NONVOTING_NODE, conn->addr.toIp().data(), m.hs.raft_port,
  ...
  case MSG_HANDSHAKE_RESPONSE:
    if (0 == m.hsr.success)
    {
      ...
      {
        peer_connection_t *nconn = __find_connection(InetAddress{m.hsr.leader_host, m.hsr.leader_port});
        if (!nconn)
        {
  ...
  case MSG_LEAVE:
  {
    ...
    int e = __append_cfg_change(self, RAFT_LOGTYPE_REMOVE_NODE, conn->addr.toIp().data(), conn->raft_port,
  ...
  case MSG_REQUESTVOTE:
  {
    ...
    e = raft_recv_requestvote((raft_server_t *)self->raft, conn->node, &m.rv, &msg.rvr);
    string fmt = "S(I$(II))";
    __peer_msg_send(conn, tpl_map(fmt.data(), &msg), bufs, buf);
  }
  break;
  case MSG_REQUESTVOTE_RESPONSE:
    e = raft_recv_requestvote_response((raft_server_t *)self->raft, conn->node, &m.rvr);
    break;
  case MSG_APPENDENTRIES:
    ...
    }
    {
      ...
      msg_t msg = {.type = MSG_APPENDENTRIES_RESPONSE};
      e = raft_recv_appendentries((raft_server_t *)self->raft, conn->node, &m.ae, &msg.aer);
  ...
  case MSG_APPENDENTRIES_RESPONSE:
    e = raft_recv_appendentries_response((raft_server_t *)self->raft, conn->node, &m.aer);
...
int __append_cfg_change(RaftService *sv, raft_logtype_e change_type, char *host, int raft_port, int http_port,
                        int node_id)
{
  // FIXME: memory leak
  SPDLOG_INFO("Appending cfg change: {} {} {} {} {}", change_type, host, raft_port, http_port, node_id);
...
int __raft_applylog(raft_server_t *raft, void *udata, raft_entry_t *ety, raft_index_t entry_idx)
{
  ...
  if (raft_entry_is_cfg_change(ety))
  {
    ...
    if (RAFT_LOGTYPE_REMOVE_NODE != ety->type || !raft_is_leader((raft_server_t *)(self->raft)))
      goto commit;
    peer_connection_t *conn = __find_connection(InetAddress{change->host, change->raft_port});

    __send_leave_response(conn);
    goto commit;
  }

  kvs.push_back(std::make_pair(std::string((char *)ety->data.buf, ety->data.len), ""));

commit:

  kvs.push_back(std::make_pair(std::string("commit_idx"), std::to_string(raft_get_commit_idx(raft))));
 ...
 * This only returns when change has been made to disk. */
int __raft_persist_term(raft_server_t *raft, void *udata, raft_term_t current_term, raft_node_id_t vote)
{
...
int __raft_logentry_offer(raft_server_t *raft, void *udata, raft_entry_t *ety, raft_index_t ety_idx)
{
  SPDLOG_INFO("__raft_logentry_offer");

  if (raft_entry_is_cfg_change(ety))
    __offer_cfg_change(raft, (const unsigned char *)ety->data.buf, (raft_logtype_e)ety->type);
  ...
  {
    SPDLOG_ERROR("Failed to update key {}, error message : {}", meta_key, e->message());
    if (auto e = self->persistenceStore.save(meta_key, value); e)
    {
      SPDLOG_ERROR("__raft_logentry_offer put metadata error: {}", e->message());
      return -1;
    }
  }
  /* 2. put entry */
  ...
  if (auto e = self->persistenceStore.save(entry_key, (char *)ety->data.buf); e)
  {
...
int __raft_node_has_sufficient_logs(raft_server_t *raft, void *user_data, raft_node_t *node)
{
  ...
  __append_cfg_change(self, RAFT_LOGTYPE_ADD_NODE, conn->addr.toIp().data(), conn->raft_port, conn->http_port,
...
int __raft_logget_node_id(raft_server_t *raft, void *user_data, raft_entry_t *entry, raft_index_t entry_idx)
{
  return self->node_id;
}

void __on_connection_accepted_by_peer(peer_connection_t *data, const TcpConnectionPtr &conn)
{
...
void __connect_to_peer(peer_connection_t *conn)
{
  ...
  self->cls_.emplace_back(std::make_unique<TcpClient>(&self->loop_, conn->addr, "RaftClient"));
  auto &cl = self->cls_.back();
  conn->connection_status = CONNECTING;
  cl->setConnectionCallback(std::bind(&__on_connection_accepted_by_peer, conn, _1));
  cl->connect();
}
...
static int __raft_send_appendentries(raft_server_t *raft, void *user_data, raft_node_t *node, msg_appendentries_t *m)
{
  ...
  if (0 < m->n_entries)
  {
    ...
    /* list of entries */
    string fmt = "IIIB";
    tpl_node *tn = tpl_map(fmt.data(), &m->entries[0].id, &m->entries[0].term, &m->entries[0].type, &tb);
    size_t sz;
    tpl_pack(tn, 0);
    tpl_dump(tn, TPL_GETSIZE, &sz);
...
RaftService::RaftService() : persistenceStore("raft", "raft_store")
{
  ...
  if (startable || joinable)
  {
    ...
    server_ = std::move(std::make_unique<muduo::net::TcpServer>(&loop_, serverAddr, "RaftServer"));
    server_->setConnectionCallback(bind(&RaftService::onConnection, this, std::placeholders::_1));
    server_->setMessageCallback(
        bind(&RaftService::onMessage, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
    if (startable)
    {
      ...
      __append_cfg_change(this, RAFT_LOGTYPE_ADD_NODE, host.data(), raft_port, http_port, node_id);
    }
    else
    {
  ...
  loop_.runEvery(PERIOD_MSEC, std::bind(raft_periodic, (raft_server_t *)raft, PERIOD_MSEC));
...
void RaftService::onConnection(const TcpConnectionPtr &conn)
{
  ...
  SPDLOG_INFO("{} -> {} is {}", conn->localAddress().toIpPort(), conn->peerAddress().toIpPort(),
  ...
  if (conn->connected())
  {
    ...
    SPDLOG_INFO("Connection established, assemble peer with coonection, addr: {}", conn1->addr.toIpPort());
 ...
 */
void RaftService::onMessage(const muduo::net::TcpConnectionPtr &conn, muduo::net::Buffer *buf,
                            muduo::Timestamp receiveTime)
{
  ...
  if (0 < buf->readableBytes())
  {
    ...
    tpl_gather(TPL_GATHER_MEM, bf.data(), bf.size(), &conn1->gt, __deserialize_and_handle_msg, conn1);
  }
}

std::optional<common::Error> RaftService::Save(const std::string &key, const std::string &value)
{
