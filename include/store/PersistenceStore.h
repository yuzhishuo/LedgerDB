...
{
public:
  PersistenceStore(std::string family = default_default_family, const std::string &name = db_name)
  ...
  {
    if (std::filesystem::exists(db_path_) || std::filesystem::create_directories(db_path_))
    {
...
private:
  void init() noexcept
  {
    ...
    auto column_families =
        std::vector{rocksdb::ColumnFamilyDescriptor(rocksdb::kDefaultColumnFamilyName, rocksdb::ColumnFamilyOptions()),
    ...
    do
    {
      ...
      // );
      if (rocksdb::Status db_status = rocksdb::DB::Open(options_, db_path_, column_families, &handles, &db_);
          !db_status.ok())
      {
    ...
    else
    {
      ...
      if (handles.size() < 2)
      {
        ...
        {
          SPDLOG_DEBUG("PersistenceStore: CreateColumnFamily {}", f_status.ToString());
        }
        assert(cf);
...
public:
   ...
   */
  virtual std::optional<common::Error> save(const std::string &key, const std::string &value) override
  {
    {
   ...
   */
  virtual std::optional<common::Error> save(const std::vector<std::pair<std::string, std::string>> &kvs) override
  {
    ...
    if (rocksdb::Status status = db_->Write(rocksdb::WriteOptions(), &batch); !status.ok())
    {
      return std::make_optional<common::Error>(status.ToString());
    }
    return std::nullopt;
  ...
  virtual std::pair<std::string, std::optional<common::Error>> load(const std::string &key) override
  {
    std::string value;

    if (rocksdb::Status status = db_->Get(rocksdb::ReadOptions(), handles[1], key, &value); !status.ok())
    {
      return std::make_pair("", std::make_optional<common::Error>(status.ToString()));
    }
    return std::make_pair(value, std::nullopt);
  }

  virtual std::optional<common::Error> delete_key(const std::string &key) override
  {
  ...
  virtual std::optional<common::Error> update_key(const std::string &key, const std::string &value) override
  {
